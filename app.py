...
def ler_config():
    ...
    if os.path.exists(CONFIG_FILE):
        ...
        password = (
            base64.b64decode(password_b64.encode("utf-8")).decode("utf-8")
            if password_b64
            else ""
        )
...
def get_encomendas(filtros):
    try:
        ...
        if conn is None:
            return (
                pd.DataFrame(),
                "Configuração da BD ausente. É necessário configurar a ligação á Base de dados.",
            )
        ...
        df = (
            pd.DataFrame.from_records(rows, columns=columns) if rows else pd.DataFrame()
        )
        return df, None
...
def configuracoes():
    ...
    if request.method == "POST":
        ...
        dsn, db, user, password = dsn_form, database_form, user_form, password_form
...
def tratamentos():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT tratamento FROM u_tratamentos (NOLOCK) order by tratamento asc"
    )
    rows = cursor.fetchall()
    cursor.close()
    conn.close()
...
@app.route("/get_clientes_tratamento")
def get_clientes_tratamento():
...
def imprimir_preview():
    ...
    cliente_clicado = request.args.get("cliente_especifico")
    ...
    if cliente_clicado and resultado:
        resultado = [
            c
            for c in resultado
            if c["cliente"].get("cliente", "").strip() == cliente_clicado.strip()
        ]
    ...
    if not resultado:
        ...
        elements.append(
            Paragraph("<b>NÃO FORAM ENCONTRADOS REGISTOS</b>", styles["Aviso"])
        )
        elements.append(Spacer(1, 12))
        elements.append(
            Paragraph(
                f"Para o tratamento: {t_ini if t_ini else 'Todos'}", styles["BodySmall"]
            )
        )
    else:
        ...
        for cliente in resultado:
            ...
            for enc in cliente["encomendas"]:
                ...
                # Encomenda
                data_encom = [
                    ["Encomenda", "", "Requisição", "Acabamento", "Micragem", "Conf"],
                    [
                        str(d.get("obrano", "") or ""),
                        data_f,
                ...
                encom_table = Table(
                    data_encom, colWidths=[60, 70, 140, 160, 50, 50], repeatRows=1
                )
                encom_table.setStyle(
                    TableStyle(
                        [
...
def detalhe():
    ...
    try:
        ...
        clientes_sel_raw = request.form.getlist("clientes_sel") or filtros.get(
            "clientes_sel", []
        )
        ...
        cursor.execute(
            "EXEC sp_listar_clientes " + ", ".join(["?"] * 13), params_comuns
        )
        clientes_lista = [str(row[0]) for row in cursor.fetchall() if row[0]]

        # Carregar lista de tratamentos
        cursor.execute(
            "EXEC sp_listar_tratamentos " + ", ".join(["?"] * 13), params_comuns
        )
        ...
        if clientes_sel or trat_sel:
            ...
            # strip á variável para que o SP retorne os resultados corretos
            valor_linha = filtros.get("linha")
...
class NumberedCanvas(pdf_canvas.Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        self.drawCentredString(
            largura / 2, 20, f"Página {self._pageNumber} de {page_count}"
        )
...
def cabecalho(canvas, doc, filtros):
    ...
    canvas.drawRightString(
        largura - 35, altura - 55, "LACOVIANA - Trat. e Lac. Alumínios de Viana, Lda"
    )
    ...
    canvas.line(
        30, altura - 115, largura - 30, altura - 115
...
def index():
    ...
    cursor.execute(
        "SELECT TOP 1 tratamento FROM u_tratamentos (NOLOCK) ORDER BY tratamento ASC"
    )
    row = cursor.fetchone()
    trat_ini_default = row[0] if row is not None else ""

    cursor.execute(
        "SELECT TOP 1 tratamento FROM u_tratamentos (NOLOCK) ORDER BY tratamento DESC"
    )
    row = cursor.fetchone()
